...
# This one is specifically built to work with the regular vl progs.
# Gives info about rnProgs
...
# these things are all kinda silly, but useful.
...
# Should work fine! # Shows RN progs
...
# e.g. 50% I, 25% V
# I believe counts gives a doubled resultt
		...
		for key in progs[m]:
			...
			for tup in progs[m][key]:
				try:
					...
					continue
				# genericIntervalDictNew = []
				# if genericIntervalDict:
				# 	#print("pc", pc, "genericIntervalDict:", genericIntervalDict)
				# 	genericIntervalDictNew = transposeGenericIntervals(genericIntervalDict, key[0])
				containsAllPitchClasses = True
				for pc in pitchClasses:
					if pc not in genericIntervalDict:
						containsAllPitchClasses = False
						#print("doesNotContain All Pitch Classes")
				if containsAllPitchClasses:
					vlList.append((key, tup[1], tup[6]))
					#print("search for interval structure: ", key, tup[6])
				# else:
				# 	print("Not included in vlList:", tup[6])
...
def transposeGenericIntervals(genericIntervals, rnStr):
	...
	# if len(genericIntervals) == 3:
		# if '9' not in newGenericIntervals:
		# 	print("rn:", rn)
		# 	print("chordRoot:", chordRoot, "rnRootInC:", rnRootInC)
		# 	print("before:", genericIntervals)
		# 	print("after: ", newGenericIntervals)
		# 	print()
...
def searchForSpecificPcPaths(pcPathList, vlList, rn1 = None, rn2 = None, inversions = False, exclusion = False):
	...
	for tup in  vlList:
		...
		for pcPath in pcPathList:
			...
			#transposedGenericIntervals = transposeGenericIntervals(tup[2], tup[0][0])
			#print("transposeGenericIntervals:", transposedGenericIntervals)
			#if relevantInterval in [interval % 12 for interval in genericIntervals]:
			if exclusion:
		...
		if shouldAppendTupFlag:
			...
			#print(pcPathList[0][0], ":", tup[2][pcPathList[0][0]], pcPathList[1][0], ":", tup[2][pcPathList[1][0]])
	return resultList
...
def main():
	...
	#doublingDict = getDoublingStats(gp, doublingDictStorage)
	doublingDict = {}
	...
	# makeTableForAGivenRnWithGetDoublingStats(doublingDict, 'viio6', numberOfVoices = 4)
	# makeTableForAGivenRnWithGetDoublingStats(doublingDict, 'ii6', numberOfVoices = 4)
	# makeTableForAGivenRnWithGetDoublingStats(doublingDict, 'ii6/5', numberOfVoices = 4)
	# makeTableForAGivenRnWithGetDoublingStats(doublingDict, 'viio7', numberOfVoices = 4)
